 ...
 * Implements Issue #1: Enhanced Metadata Collection
 *
 * @author Andy Edwards
 * @version [0.3.0] - 2025-11-21
 */
...
var DriveApiHelpers = (function () {
  ...
  /**
   * Enhanced metadata fields for Issue #1
   * Includes all fields needed for comprehensive file analysis
   */
  ...
  /**
   * Fetches all file and folder data from specified root with enhanced metadata
  ...
  /**
   * Queries Drive API for items in specified folders with enhanced metadata
  ...
  function queryDriveApi_(folders, pageToken, driveId, corpora) {
    ...
    const requestPayload = {
      q: queryString,
      fields: `items(${ENHANCED_FIELDS}), nextPageToken`,
      supportsAllDrives: true,
   ...
   * @param {Array<Object>} items - Drive API items with enhanced metadata
  ...
  function processItemsIntoRows_(items, parentFolders, folderPathLookup) {
    ...
    items.forEach(item => {
      ...
      // Build enhanced row data with all metadata
      const row = buildEnhancedRow_(item, parentFolder, isFolder, folderPathLookup);
      rows.push(row);
  ...
  /**
   * Builds enhanced row data with all metadata fields
   * @param {Object} item - Drive API item
   * @param {Object} parentFolder - Parent folder object
   * @param {boolean} isFolder - Whether item is a folder
   * @param {Map} folderPathLookup - Folder path lookup
   * @returns {Array} Enhanced row data
   * @private
   */
  function buildEnhancedRow_(item, parentFolder, isFolder, folderPathLookup) {
    const icon = isFolder ? '📂' : '📃';
    const fileSize = item.fileSize || '0';
    const createdDate = item.createdDate || '';
    const modifiedDate = item.modifiedDate || '';
    const lastViewedDate = item.lastViewedByMeDate || '';
    const ownerName = item.ownerNames ? item.ownerNames[0] : '';
    const ownerEmail = item.owners && item.owners.length > 0 ? item.owners[0].emailAddress : '';
    const isShared = item.shared || false;
    const isStarred = item.labels && item.labels.starred ? item.labels.starred : false;
    const fileExtension = item.fileExtension || '';
    const thumbnailLink = item.thumbnailLink || '';
    const alternateLink = item.alternateLink || '';
    const parentPath = folderPathLookup.get(parentFolder.id) || parentFolder.name;
